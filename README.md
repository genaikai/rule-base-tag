...
필드 이름은 `schema.py` 의 상수를 가리킨다 — `QUERY` = `input_msg_content`,
`ANSWER` = `output_msg_content`. 판정은 컬럼 이름을 직접 쓰지 않으므로 로그의
이름이 바뀌면 그 두 줄만 고친다.
...
| Empty Retrieved Docs | `features/empty_retrieved_docs/` | `RETRIEVED_DOCS` — 컬럼 없음 |
| Empty Search Query | `features/empty_search_query/` | `SEARCH_QUERY` — 컬럼 없음 |
...
뒤의 둘은 읽을 컬럼이 로그에 아직 없다. 코드는 그대로 두고 리포트에
`n/a (컬럼 없음)` 으로 뜬다 — `0` 으로 찍으면 "봤는데 없었다" 와 구별되지 않는다.
컬럼이 붙는 날 `schema.py` 의 `INPUT_SCHEMA` 에 필드를 더하면 그대로 살아난다.
...
- **컬럼 이름은 받았지만 값은 아직 못 봤다.** `INPUT_SCHEMA` 의 36개는 실제 로그의
  컬럼 목록이다. 널 허용 여부·허용값·범위는 아직 추정이라 `note` 에 "확인 필요" 로
  적어두었다. 실데이터로 한 번 돌리면 `schema` 줄에 어긋난 것이 이름과 숫자로
  뜬다 — 그게 첫 사이클의 수확이다
- **판정이 읽는 컬럼은 둘뿐이다.** `input_msg_content` 와 `output_msg_content`.
  나머지 34개는 `used=False` 라 어긋나도 노트로만 뜬다. 판정이 하나를 읽기
  시작하면 그 줄만 `used=True` 로 올린다
