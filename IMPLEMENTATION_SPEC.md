...
`src/rule_base_tag/contracts.py`의 `INPUT_SCHEMA`를 갈아끼우면 시작이다.
...
```
src/<pkg>/
  contracts.py  load.py  report.py  synth.py   ← 공유. 기능이 늘어도 하나뿐이다
...
(`from ..contracts` → `from ...contracts`). 안 고치면 `ModuleNotFoundError` 로 죽는다.
...
공유 코드(`contracts` · `load` · `report` · `synth`)와 진입점은 기능이 늘어도 손대지 않는다.
...
**안 쓰는 필드의 어긋남은 위반이 아니라 노트다.** 파이프라인이 읽지 않는 필드가 계약과
달라도 판정은 멀쩡하다. 그걸 `MISMATCH` 로 올리면 매 실행마다 같은 줄이 뜨고, **사람은
곧 그 줄 자체를 안 보게 된다.** `contract` 줄은 "판정이 틀렸을 수 있다"는 뜻이어야 한다 —
...
|---|---|---|
| `0` | 정상 | 다음으로 |
| `1` | **돌았지만 온전치 않다** (계약 위반, 볼 것이 없음) | 재시도해도 같다. 사람이 본다 |
